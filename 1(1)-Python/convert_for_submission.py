...
def integrate_file(n: str) -> None:
    ...
    lib_code = open(f"{ROOT_PATH[n]}/lib.py", encoding='utf-8').read() #error가 떠서 수정했어요 - 최성민
    integrated_code = lib_code + "\n\n\n" + num_code

    if n == "1629": # 1629는 lib code 통합 제외
        integrated_code = num_code 
    folder_num = ROOT_PATH[n][2]
    open(f"{PATH_SUB}/{folder_num}_{n}.py", 'w', encoding='utf-8').write(integrated_code) #UTF-8만 추가 - 최성민
