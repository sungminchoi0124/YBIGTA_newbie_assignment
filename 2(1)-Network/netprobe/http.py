...
def parse_status_and_preview(raw: bytes, max_preview: int = 200) -> tuple[Optional[int], str, Optional[str]]:
    ...
    try:
        ...
        if boundary == -1:
            return None, "", "ERROR: no boundary"

        raw_header = raw[:boundary]
        raw_body = raw[boundary + 4:]
        ...
        preview_bytes = raw_body[:max_preview]
        preview = preview_bytes.decode("utf-8", errors="replace")

    except ValueError:
        return None, "", "ERROR: Invalid status code format"
